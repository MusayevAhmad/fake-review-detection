...
try:
    print("\nLoading trained TF-IDF vectorizer and SVM model (with POS tagging)...")
    tfidf_vectorizer = joblib.load(f'{models_dir}/svm_ngram_pos_vectorizer.joblib')
    best_svm = joblib.load(f'{models_dir}/best_svm_ngram_pos.joblib')
...
print("\n🔹 Test Set Evaluation (SVM with POS Tagging):")
print(test_report)
...
plt.ylabel('True')
plt.title('Confusion Matrix (Test Set - SVM with POS Tagging)')
plt.tight_layout()
plt.savefig(f'{results_dir}/confusion_matrix_svm_ngrams_pos_test.png')
print(f"\nConfusion matrix saved as '{results_dir}/confusion_matrix_svm_ngrams_pos_test.png'")

# Save test results to a file
with open(f'{results_dir}/svm_ngram_pos_test_results.txt', 'w') as f:
    f.write(f"Test Accuracy (SVM with POS Tagging): {test_accuracy:.4f}\n\n")
    f.write("Classification Report:\n")
    f.write(test_report)
    
print(f"\nTest results saved to '{results_dir}/svm_ngram_pos_test_results.txt'")
print("\nSVM with TF-IDF N-grams and POS Tagging Test Evaluation Complete!")